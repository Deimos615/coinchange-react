...
export const SignUp = () => {
  ...
  const onSubmit = async (e) => {
    ...
    if (Object.keys(validationErrors).length === 0) {
      try {
        const response = await commonAxios.post(`/auth/signup`, e)
                ...
                <div className="signup_frm_wrap">
                  <form onSubmit={handleSubmit(onSubmit)} action='#'>
                    ...
                    <div className="form-group">
                      <input {...register('username')} type="text" className="form-control" placeholder="Enter Username" required="" data-listener-added_edb9d59a="true" />
                      <i className="fa fa-user"></i>
                      {errors.username && <div className="text-red-500 text-sm">{errors.username}</div>}
                    </div>
                    <div className="form-group">
                      <input {...register('email')} className="form-control" placeholder="Enter Email Address" required="" data-listener-added_edb9d59a="true" />
